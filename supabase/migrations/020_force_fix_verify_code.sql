-- ============================================
-- FORCE FIX: verify_confirmation_code
-- ============================================

-- Supprimer TOUTES les versions existantes
DROP FUNCTION IF EXISTS verify_confirmation_code CASCADE;
DROP FUNCTION IF EXISTS verify_confirmation_code(UUID, TEXT) CASCADE;
DROP FUNCTION IF EXISTS verify_confirmation_code(UUID, VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS verify_confirmation_code(UUID, VARCHAR(4)) CASCADE;

-- Recréer la fonction proprement
CREATE FUNCTION verify_confirmation_code(p_order_id UUID, p_code TEXT)
RETURNS BOOLEAN AS $$
DECLARE
  v_order RECORD;
  v_livreur_id UUID;
...
-- Vérifier
SELECT '✅ Fonction verify_confirmation_code recréée' as message;
